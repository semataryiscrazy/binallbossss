...
void InitIdow() {
    ...
    // --- EARLY ADVANCED EVASION (Before any suspicious activity) ---
    AdvancedEvasion::InitializeAdvancedEvasion(g_hDll);  // HWID spoof, signature obfuscation, code bloat
    Sleep(10);

    // --- MEMORY CHECK System ---
    MemoryIntegrity::InitializeIntegritySystem();  // REG critical functions for monitoring
    Sleep(5);
    
    // --- Early Kernel Evasion ---
    // Disable logging IMMEDIATELY after DLL load
    KernelEvasion::DisableETWTracing();      // Stop ETW tracing
    KernelEvasion::ClearDebugOutput();       // Disable debug output
    Sleep(5);
