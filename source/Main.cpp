                        ...
                        if (strlen(Auth.Usuario) > 0 && strlen(Auth.Senha) > 0) {
                            std::thread([=]() {
                                ...
                                if (ok) {
                                    ...
                                    std::thread(NetworkInit).detach();
                            std::thread([]() { __try { Sleep(2000); diag_log("StartThread: begin"); LoadLibraryAndHook(); diag_log("StartThread: Hook done, starting features"); } __except(EXCEPTION_EXECUTE_HANDLER) { diag_log("StartThread: __except caught crash"); } }).detach();
                                } else {
