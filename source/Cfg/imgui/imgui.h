...
namespace ImGui
{
    ...
    IMGUI_API bool          TabHeader(const char* tabname, int* currentTab, std::vector<const char*> Tabs, int TabIndex);   // button
    IMGUI_API bool          TabHeaderButton(const char* ButtonName, bool enabled, int direction, int TabIndex);
    ...
    IMGUI_API bool          KeyBind(const char* label, int* key, int* KeyState);
    ...
    IMGUI_API bool          BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags = 0, int itemcount = 0);
...
struct ImDrawList
{
    ...
    IMGUI_API void  AddRectFilledMultiColor(const ImVec2& p_min, const ImVec2& p_max, ImU32 col_upr_left, ImU32 col_upr_right, ImU32 col_bot_right, ImU32 col_bot_left, float rounding = 5, ImDrawFlags flags = 0);
