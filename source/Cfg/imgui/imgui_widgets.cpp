...
extern ImFont* FontAwesomeRegular;
extern ImFont* FontAwesomeSolid;
extern ImFont* FontAwesomeSolid14;
extern ImFont* FontAwesomeBrands;

extern ImFont* InterBlack;
extern ImFont* InterBold;
extern ImFont* InterBold12;
extern ImFont* InterExtraBold;
extern ImFont* InterExtraLight;
extern ImFont* InterLight;
extern ImFont* InterMedium;
extern ImFont* InterRegular;
extern ImFont* InterSemiBold;
extern ImFont* InterThin;
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.b = ImLerp(it_anim->second.b, hovered ? ImColor(23, 0, 173, 200).Value : ImColor(ImGui::GetColorU32(ImGuiCol_Border)).Value, g.IO.DeltaTime * 10.f);
...
bool ImGui::Tab(const char* Label, const char* Icon, bool Enabled)
{
    ...
    const ImVec2 size = ImVec2(label_size.x + 30, 20); // Dynamic width based on label
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect bb(pos, pos + size);
    ...
    if (anims.find(id) == anims.end()) anims[id] = 0.0f;
    anims[id] = ImLerp(anims[id], Enabled ? 1.0f : 0.0f, 0.1f);

    static float animated_dot_x = 0;
    static float target_dot_x = 0;
    static bool dot_init = false;

    if (Enabled)
    {
        target_dot_x = bb.Min.x + bb.GetWidth() / 2.f;
        if (!dot_init) {
            animated_dot_x = target_dot_x;
            dot_init = true;
        }
    }
    animated_dot_x = ImLerp(animated_dot_x, target_dot_x, 0.1f);

    // Background for active tab (rounded square, back to Pink)
    if (anims[id] > 0.01f)
    {
        ImU32 bg_col = ImColor(219, 0, 166, (int)(anims[id] * 50)); 
    }

    // Sliding line below the icons (horizontal) - Match image
    if (Enabled || std::abs(animated_dot_x - target_dot_x) > 0.1f)
    {
        float line_width = 20.0f;
        window->DrawList->AddRectFilled(ImVec2(animated_dot_x - line_width/2, bb.Max.y - 2), ImVec2(animated_dot_x + line_width/2, bb.Max.y), ImColor(219, 0, 166, 255), 10);
    }

    // Text Label
    ImU32 text_col = Enabled ? ImColor(255, 255, 255, 255) : ImColor(255, 255, 255, 100);
    window->DrawList->AddText(bb.Min + (size - label_size) * 0.5f, text_col, Label);
...
bool ImGui::TabHeaderButton(const char* ButtonName, bool enabled, int direction, int TabIndex)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = ImVec2(label_size.x + 30, 35); // Slightly wider and shorter
    ...
    // Pink background like in the image
    if (anims[id] > 0.01f)
    {
        ImU32 bg_col = ImColor(219, 0, 166, (int)(anims[id] * 255));
        window->DrawList->AddRectFilled(bb.Min, bb.Max, bg_col, 5);
    }

    ImU32 text_col = enabled ? ImColor(255, 255, 255, 255) : ImColor(255, 255, 255, 100);
    window->DrawList->AddText(bb.Min + (size - label_size) * 0.5f, text_col, ButtonName);
...
bool ImGui::TabHeader(const char* tabname, int* currentTab, std::vector<const char*> Tabs, int TabIndex)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    // Position it higher up
    SetCursorPos(ImVec2(20, 5));
    ...
    {
        WidhtMax += 30 + CalcTextSize(Tabs.at(i)).x + (i > 0 ? 10 : 0);
    }
    PopFont();

    SetCursorPosX(20 + (window->Size.x - 40 - WidhtMax) / 2);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    ImRect bb = bb_frame;
    bb.Expand(ImVec2(-ImClamp(IM_TRUNC((bb_frame_width - 2.0f) * 0.5f), 0.0f, 3.0f), -ImClamp(IM_TRUNC((bb_frame_height) * 0.5f), 0.0f, 3.0f)));
    ...
    if (axis == ImGuiAxis_Y)
        window->DrawList->AddRectFilledMultiColor(grab_rect.Min, grab_rect.Max, ImColor(23, 0, 173, 0), ImColor(23, 0, 173, 0), ImColor(23, 0, 173, (int)(GetStyle().Alpha * 255)), ImColor(23, 0, 173, (int)(GetStyle().Alpha * 255)), 5);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(GetWindowSize().x - 20, 25));
    ...
    if (Value == Values.end())
    {
        CheckBoxAnim new_anim = { 1.0f, 0.0f };
        Values.insert({ id, new_anim });
        Value = Values.find(id);
    }


    Value->second.a = ImLerp(Value->second.a, *v ? 0.f : 1.f, 0.1f);
    Value->second.brightness = ImLerp(Value->second.brightness, *v ? 1.0f : 0.4f, 0.1f);


    const ImRect check_bb(total_bb.Max - ImVec2(25, 25), total_bb.Max);
    ...
    if (std::abs(Value->second.a) < 0.9f)
        window->DrawList->AddRectFilled(check_bb.Min + ImVec2(check_bb.GetWidth() / 2 * Value->second.a + 1, check_bb.GetHeight() / 2 * Value->second.a + 1), check_bb.Max - ImVec2(check_bb.GetWidth() / 2 * Value->second.a + 1, check_bb.GetHeight() / 2 * Value->second.a + 1), ImColor(219, 0, 166, (int)(GetStyle().Alpha * 255)), 5);
    ...
    if (std::abs(Value->second.a) < 0.7f)
        RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(square_sz / 2, square_sz / 2) - ImVec2(checkmark_size / 2, checkmark_size / 2), ImColor(255, 255, 255, (int)(GetStyle().Alpha * 255)), checkmark_size);

    if (label_size.x > 0.0f)
    {
        ImU32 text_color = ImColor(1.0f, 1.0f, 1.0f, Value->second.brightness * GetStyle().Alpha);
        window->DrawList->AddText(ImVec2(total_bb.Min.x, total_bb.Min.y + total_bb.GetHeight() / 2 - label_size.y / 2), text_color, label);
    }
...
bool ImGui::KeyBind(const char* label, int* Key, int* KeyState)
{
    ...
    if (it_anim == anim.end())
    {
        KeyBindAnim new_anim = { 0.0f, ImVec4(1.f, 1.f, 1.f, 1.f) };
        anim.insert({ id, new_anim });
        it_anim = anim.find(id);
    }
    ...
    {
        _snprintf(buf_display, sizeof(buf_display), "%s", KeyNames[*Key]);
    }
    else if (g.ActiveId == id)
    {
        _snprintf(buf_display, sizeof(buf_display), "Press");
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int itemcount)
{
    ...
    if (!(flags & ImGuiComboFlags_NoPreview))
    {
        ...
        window->DrawList->AddCircleFilled(ImVec2(bb.Max.x - 14, bb.Min.y + total_bb.GetHeight() / 2), Value->second.a, ImColor(23, 0, 173, (int)(GetStyle().Alpha * 255)), 360);
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    Value->second.anim = ImLerp(Value->second.anim, (float)percent / 100.f * frame_bb.GetWidth(), 0.1f);
    Value->second.anim2 = ImLerp(Value->second.anim2, (float)percent, 0.1f);

    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(14, 14, 14, (int)(GetStyle().Alpha * 255)), 5);

    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(1, 0), ImVec2(frame_bb.Min.x + Value->second.anim + 1, frame_bb.Max.y), ImColor(23, 0, 173, (int)(GetStyle().Alpha * 255)), 5);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() || hovered ? ImColor(23, 23, 23) : ImColor(23, 23, 23), g.IO.DeltaTime * 6.f);
    it_anim->second.border = ImLerp(it_anim->second.border, IsItemActive() || hovered ? ImColor(23, 0, 173, 255) : ImColor(30, 30, 30), g.IO.DeltaTime * 3.f);

    // Render frame
    if (!is_multiline)
    {
        GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(it_anim->second.background), 4.f);
        ImGui::GetWindowDrawList()->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(it_anim->second.border), 4.0f, ImDrawFlags_RoundCornersAll, 1.0f);
    }

    const ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + inner_size.x, frame_bb.Min.y + inner_size.y); // Not using frame_bb.Max because we have adjusted size
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + ImVec2(8, 10);
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            ImU32 bg_color = ImColor(10, 10, 10, 50); // FIXME: current code flow mandate that render_cursor is always true here, we are leaving the transparent one for tests.
        ...
        if (render_cursor)
        {
            ...
            it_anim->second.slow = ImLerp(it_anim->second.slow, cursor_screen_rect.Min.x - total_bb.Min.x, g.IO.DeltaTime * 10.0f);

            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(total_bb.Min + ImVec2(it_anim->second.slow, 8), total_bb.GetBL() - ImVec2(-it_anim->second.slow, 8), GetColorU32(ImGuiCol_Text));
